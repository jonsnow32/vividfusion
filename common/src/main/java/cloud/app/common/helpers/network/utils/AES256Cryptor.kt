...
object AES256Cryptor {
    ...
    fun encrypt(plaintext: String, passphrase: String): String? {
        try {
            ...
            EvpKDF(passphrase.toByteArray(charset("UTF-8")), keySize, ivSize, saltBytes, key, iv)

            // Actual encrypt
            val cipher = Cipher.getInstance("AES/CBC/PKCS5Padding")
            cipher.init(Cipher.ENCRYPT_MODE, SecretKeySpec(key, "AES"), IvParameterSpec(iv))
            val cipherBytes = cipher.doFinal(plaintext.toByteArray(charset("UTF-8")))
    ...
    fun decrypt(ciphertext: String, passphrase: String): String {
        try {
            ...
            val ctBytes = Base64.decode(ciphertext.toByteArray(charset("UTF-8")), Base64.DEFAULT)
