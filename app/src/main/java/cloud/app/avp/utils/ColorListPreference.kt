...
import cloud.app.avp.databinding.ItemColorCardBinding
...
class ColorListPreference(val fragment: Fragment, var listener: Listener? = null) :
    ...
    private fun getSelectedColor(): Int? {
        val color = sharedPreferences.getInt(key, -1)
    ...
    private fun getSavedColors(): List<Pair<Boolean, Int>> {
        ...
        return colorString?.split(",")?.mapNotNull {
            ...
            val color = it.toIntOrNull() ?: return@mapNotNull null
            Pair(color == selectedColor, color)
        } ?: emptyList()
    }
