...
import cloud.app.vvf.extension.InstallationUtils.uninstallApp
import cloud.app.vvf.extension.InstallationUtils.uninstallFile
...
class ExtensionViewModel @Inject constructor(
    ...
    suspend fun uninstall(
        ...
        val appResult = runCatching {
            uninstallApp(context, extension.metadata.path)
        }.exceptionOrNull()
        val result = if (extension.metadata.importType == ImportType.App) appResult else fileResult
        if (result == null) messageFlow.emit(Message(context.getString(R.string.extension_uninstalled_successfully)))
        else throwableFlow.emit(result)
    }
    ...
    suspend fun addExtension(context: FragmentActivity, extension: ExtensionAssetResponse) =
        flow<InstallStatus> {
            ...
            val result = InstallStatus.FAILED //context.installExtension(file.toUri().toString())
            emit(result)
        }
...
//      val key = "${type.name}/${extensionMetadata.className}"
            voteMutex.withLock {
                ...
                if (!isVoted) {
                    val response =
                        votingService.vote(type.name, extensionMetadata.className).execute()
