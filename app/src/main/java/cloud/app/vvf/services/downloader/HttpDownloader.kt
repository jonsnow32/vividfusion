...
class HttpDownloader @AssistedInject constructor(
  ...
  override suspend fun doWork(): Result {
    ...
    Timber.Forest.d("Starting download: $downloadId | $downloadUrl | $fileName | Type: $downloadType | Resume: $isResuming (${resumeProgress}%, ${resumeBytes} bytes)")
  ...
  private suspend fun downloadHttpFile(
    ...
    return try {
      ...
      Timber.Forest.e(e, "Download failed for $downloadId")
      Result.failure(
        workDataOf(
  ...
  private suspend fun downloadFile(
    ...
    if (isStopped) {
      Timber.Forest.d("Download $downloadId: Work is stopped, returning early")
    ...
    if (isResuming && resumeBytes > 0) {
      ...
      Timber.Forest.d("Download $downloadId: Resuming from byte $resumeBytes")
    }
    ...
    if (isStopped) {
      response.close()
      Timber.Forest.d("Download $downloadId: Work stopped after HTTP request")
      return Result.failure(
        workDataOf(
          "error" to "Download was stopped",
          "downloadId" to downloadId,
    ...
    if (!response.isSuccessful) {
      if (isResuming && response.code == 416) {
        ...
        Timber.Forest.w("Download $downloadId: Range not satisfiable, file might be complete")
    ...
    val mediaFile = if (isResuming && resumeBytes > 0) {
      ...
      try {
        ...
        if (existingFile != null) {
          ...
          Timber.Forest.d("Download $downloadId: Found existing file, size: $existingSize, expected resume from: $resumeBytes")

          if (existingSize >= resumeBytes) {
            // File exists and has expected size or more
            if (existingSize > resumeBytes) {
              Timber.Forest.w("Download $downloadId: Existing file is larger than expected resume point, truncating to $resumeBytes")
              // Truncate file to resume point if it's larger
              truncateFile(existingFile, resumeBytes)
            }
            existingFile
          } else {
            Timber.Forest.w("Download $downloadId: Existing file is smaller than resume point, starting fresh")
          ...
          Timber.Forest.w("Download $downloadId: No existing file found for resume, starting fresh")
          responseBody.contentType()?.toString()
            ?.let { downloadsDir.createFile(fullFileName, it) }
            ?: throw IOException("Failed to create media file")
        }
      } catch (e: Exception) {
        Timber.Forest.w(e, "Download $downloadId: Error accessing existing file, creating new one")
        responseBody.contentType()?.toString()
          ?.let { downloadsDir.createFile(fullFileName, it) }
          ?: throw IOException("Failed to create media file")
      }
    ...
    Timber.Forest.d("Download $downloadId: Starting file write - Response code: ${response.code}, Content length: $contentLength, Total bytes: $totalBytes, Resume from: $resumeBytes")
    ...
    try {
        ...
        if (shouldAppend) {
          mediaFile.openOutputStream(true).use { output -> // true for append mode
            ...
            while (input.read(buffer).also { bytesRead = it } != -1) {
              ...
              if (iterationCount % 100 == 0 && isStopped) {
                Timber.Forest.d("Download $downloadId: Work stopped during file write (resume mode)")
                return Result.failure(
                  workDataOf(
                    "error" to "Download was stopped",
                    "downloadId" to downloadId,
              ...
              if (iterationCount % 50 == 0) {
                ...
                val progressPercent =
                  if (totalBytes > 0) (actualTotal * 100 / totalBytes).toInt() else 0
          ...
          mediaFile.openOutputStream().use { output ->
            ...
            while (input.read(buffer).also { bytesRead = it } != -1) {
              // Check for cancellation every few iterations to be responsive
              if (iterationCount % 100 == 0 && isStopped) {
                Timber.Forest.d("Download $downloadId: Work stopped during file write (normal mode)")
                return Result.failure(
                  workDataOf(
                    "error" to "Download was stopped",
                    "downloadId" to downloadId,
              ...
              if (iterationCount % 50 == 0) {
                val progressPercent =
                  if (totalBytes > 0) (totalBytesWritten * 100 / totalBytes).toInt() else 0
      ...
      Timber.Forest.d("Download $downloadId: File write completed - Final size: $finalSize, Expected: $expectedSize, Written: $totalBytesWritten")
      ...
      Timber.Forest.e(e, "Download $downloadId: Error during file write")
      return Result.failure(
        workDataOf(
  ...
  private fun truncateFile(file: KUniFile, targetSize: Long) {
    try {
      ...
      Timber.Forest.d("Truncated file to $targetSize bytes")
    } catch (e: Exception) {
      Timber.Forest.e(e, "Failed to truncate file")
      throw e
    }
  }
    ...
    return try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        ...
        if (appSpecificDownloads != null) {
          Timber.Forest.d("Using app-specific Downloads directory: ${appSpecificDownloads.absolutePath}")
      ...
      if (hasWriteExternalStoragePermission()) {
        ...
        if (publicDownloads != null && (publicDownloads.exists() || publicDownloads.mkdirs())) {
          Timber.Forest.d("Using public Downloads directory: ${publicDownloads.absolutePath}")
          return KUniFile.Companion.fromFile(context, publicDownloads)
      ...
      Timber.Forest.d("Using fallback directory: ${internalDownloads.absolutePath}")
      KUniFile.Companion.fromFile(context, internalDownloads)
        ?: throw IOException("Failed to create KUniFile from fallback directory")

    } catch (e: Exception) {
      Timber.Forest.e(e, "Error accessing downloads directory, using internal storage")
