...
class AdTestFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    applyInsets { insets ->
      binding.root.setPadding(insets.start, insets.top, insets.end, insets.bottom)
    }
    setupBannerAd()
    setupClickListeners()
    startStatusUpdates()
  ...
  private fun updateActionCount() {
    ...
    binding.btnIncrementAction.text =
      if (actionsUntilNextAd == 3) "+ Action (Next ad: now)" else "+ Action ($actionsUntilNextAd until ad)"
  }
