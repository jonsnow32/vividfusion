...
class AdWaterfallManager @Inject constructor() {
  ...
  private val preloadQueue =
    ConcurrentHashMap<String, Long>() // provider_adtype -> last_preload_time
  ...
  private suspend fun preloadAdsForProvider(context: Context, provider: AdProvider) =
   ...
   */
  suspend fun forcePreloadAdType(context: Context, adType: AdProvider.AdType) =
    withContext(Dispatchers.IO) {
      if (!isInitialized) return@withContext
